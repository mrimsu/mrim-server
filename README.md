...
Проект лицензирован по лицензии GNU AGPL версии 3. Учтите это, если хотите сделать форк проекта.

> [!CAUTION]
> Проект Renaissance никак не связан с Mail.ru Group, компанией ООО "ВК" и его структурами. Проект разрабатывается независимыми разработчиками.
...
- Node.js v18 или выше, тестируется на v22 LTS
- MariaDB (приоритетнее) или любой другой SQL сервер

На FreeBSD и его производных сервер не запускается из коробки из-за Node.js модуля `sharp`. Модуль придётся собирать ручками. 

## Процедура установки

1. Установите Node.js, npm, MariaDB или Percona
...
Сервер можно обернуть в сервис на любых системах инилиализации (systemd, OpenRC и пр.).
...
- Вход в аккаунт
- Смена статуса 
- Xstatus (Агент 5.0 и выше; протокол MRIM 1.14 и выше)
- Отправка сообщений, мультиков и будильников
- Отправка SMS
- Аватарки (образы)
- Поиск пользователей
- Просмотр анкет
- Игры (на версиях Агента 4.2 до 5.2)
- Микроблог
- Прокси для обмена данными в условиях NAT
- Звонки (до Агента 5.1 включительно)
- Отправка файлов
...
- Отправка файлов на Android версии Агента
- Серверная история сообщений и их отправка на очень поздних версиях Агента для Android

## Почему GNU AGPL v3?

Не хочу, чтобы умирал проект. Проприетарный протокол должен быть доступен всему миру и всеми силами - хоть человеком, хорошо разбирающимся в ассемблере, хоть ИИ, который хоть и сгенерирует жидкую шизофрению, но обязательно натолкнёт на правильный результат.
