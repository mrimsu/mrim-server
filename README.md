...
Сервер имитирует работу протокола версии 1.0 до 1.23. Более младшие версии пока не работают с этим сервером, а, возможно, они где-то затерялись в глубинках Архива Интернета.
...
| ------ | ------ | --------- | ----------- |
| 🟠 | Агент@Mail.ru версии 2.0 (сборка 217) | Windows | Работает через hosts. При отправке сообщения вываливается из аккаунта. Список и поиск контактов не работает. |
| 🟠 | Агент@Mail.ru версии 2.55 (сборка 423) | Windows | Работает через hosts. Не получает сообщения. При отправке сообщения вываливается из аккаунта. Список и поиск контактов не работает |
| 🟡 | Агент@Mail.ru версии 4.0 (сборка 768) | Windows | Работает через hosts. Список и поиск контактов не работает |
...
| 🔴 | Агент@Mail.ru 2.0.0 | Windows Mobile | Отправляет неверный `MRIM_CS_LOGIN2` с отсутствиющим LPS с Useragent. |
| ✅ | Агент@Mail.ru версии 2.2 | Android | Работает. Необходим патч клиента со сменой адреса. |
| ✅ | Агент@Mail.ru версии 3.2 | Android | Работает. |
...
На очень старых клиентах:

- Добавление в контакты (MRIM 1.4 и ниже)
- Сообщения (MRIM 1.0)
- Список контактов (до MRIM 1.8)
